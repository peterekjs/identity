...
function mergeTypes<T extends TypeDescription<any>[]>(...descriptions: T): T[number] extends TypeDescription<infer S> ? TypeDescription<S> : never {
  ...
  return {
    ...
    get isObject() { return descriptions.some(x => x.isObject) },
    get props() { return descriptions.map(x => x.props).flat() },
