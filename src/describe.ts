...
function describeType<T>(
  ...
  props: T extends NonNullable<unknown>
    ? (keyof T)[]
    : never[] = [] as T extends NonNullable<unknown> ? (keyof T)[] : never[]
): TypeDescription<T> {
  return {
    name,
    validate,
...
function describeInstance<T>(
  Ctor: new () => T,
  instantiator: () => T = () => new Ctor()
): TypeDescription<T> {
  const props = Object.keys(
    instantiator() ?? Object.getOwnPropertyDescriptors(Ctor).prototype.value
  ) as (keyof T)[]
  ...
  return {
    ...
    equals(a, b) {
      return (
        ...
        Array.isArray(b) &&
        a.length === b.length &&
        a.every((v, i) => description.equals(v, b[i]))
      )
    },
    get isObject() {
      return false
...
const hasOwn = <T extends NonNullable<unknown>>(
  input: object,
  key: keyof T
): input is T => Object.hasOwn(input, key)
...
function describeObject<P extends PropDescriptions<NonNullable<unknown>>>(
  ...
  const props = Object.keys(propDescriptions) as (T extends NonNullable<unknown>
    ? keyof T
    : never)[]
  const getPropEntries = () =>
    Object.entries(propDescriptions) as [keyof T, TypeDescription<T[keyof T]>][]

  function validate(input: unknown): input is T {
    return (
      isObject(input) &&
      getPropEntries().every(
        ([k, v]) => hasOwn(input, k) && v.validate(input[k])
      )
    )
  }
  ...
  function equals(a: T, b: T) {
    return (
      isObject(a) &&
      isObject(b) &&
      getPropEntries().every(([k, v]) => v.equals(a[k], b[k]))
    )
  }
