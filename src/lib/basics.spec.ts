...
import { $any, $array, $assignable, $bigint, $boolean, $defined, $function, $nil, $null, $number, $object, $primitive, $string, $symbol, $undefined } from './basics'

testIdentity($bigint, pick(['bigint']))
testIdentity($boolean, pick(['boolean (false)', 'boolean (true)']))
testIdentity($function, pick([
  'function', 'function (throws exception)', 'lambda', 'lambda (returns value)'
]))
testIdentity($number, pick([
  'zero', 'negative zero', 'integer', 'negative integer', 'float', 'negative float',
  'infinity', 'negative infinity', 'NaN'
]))
testIdentity($object, pick([
  'empty object', 'non-empty object', 'iterable', 'async iterable',
  'disposable', 'async disposable', 'empty array', 'non-empty array',
  'Map', 'Set', 'WeakMap', 'WeakSet', 'Date', 'Date (invalid)', 'Error'
]))
