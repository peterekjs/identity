...
const $NaN: TypeIdentity<typeof NaN> = defineIdentity(
  describeType(
    'NaN',
    (v: unknown): v is typeof NaN => $number.is(v) && isNaN(v)
  )
)
...
const $finiteNumber: TypeIdentity<number> = defineIdentity(
  describeType(
    'finite number',
    (v): v is number => $number.is(v) && Number.isFinite(v)
  )
)
...
const $zero: TypeIdentity<number> = defineIdentity(
  describeType('zero', (v): v is number => v === 0)
)
